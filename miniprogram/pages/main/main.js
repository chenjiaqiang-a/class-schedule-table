...
Page({
  ...
  onAddClass:function(event){
    ...
    for(let i = 0; i < table.length; k++) {
      if (table[i]._id === _id) {
        break
      }
    } 
    if (table[i].timeAndPlaceList.classid !== event.timeAndPlaceList.classid){
      db.collection('classTable').add({
        data: {
          ...
          timeAndPlaceList:[{
            ...
            teachingBuildingName: event.teachingBuildingName,
        ...
        success: (res) => {
          ...
          const classSet = table.concat(this.data.classSet)
          this.setData({
            classSet:classSet
          })
        }
  ...
  onChangeClass:function(_id, classid){
    const db = wx.cloud.database
